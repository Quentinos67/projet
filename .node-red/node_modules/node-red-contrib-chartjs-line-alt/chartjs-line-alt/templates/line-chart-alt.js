document.addEventListener("DOMContentLoaded", function(event) {
    ...
    socket.on(topic, function(red){
        ...
        if (red.config !== undefined) {
			...
			config.options.scales['yAxes'][0].ticks.min = Number(red.config.yminimum);
			config.options.scales['yAxes'][0].ticks.max = Number(red.config.ymaximum);			
			config.options.animation.duration = red.config.aniduration;
			
/*			if(chart.config.data.datasets.length > 0)
				chart.config.data.datasets[0].animation.duration = red.config.aniduration;
*/
            // refresh chart
            chart.update();
        }
